...
@Repository
public interface McCuentaDao extends JpaRepository<McCuenta, Integer> {

  @Query("SELECT c FROM McCuenta c WHERE c.id = :id AND c.empresaId = :empresaId")
  Optional<McCuenta> findByIdAndEmpresaId(
      @Param("id") Integer id, @Param("empresaId") Integer empresaId);

  /** Nivel 1: cuentas sin padre (raíces del árbol), paginadas. */
  @Query(
      "SELECT c FROM McCuenta c WHERE c.empresaId = :empresaId AND c.cuentaPadreId IS NULL "
          + "ORDER BY c.id ASC")
  Page<McCuenta> findRaices(@Param("empresaId") Integer empresaId, Pageable pageable);

  /** Hijos directos de una cuenta — cargados lazy al expandir la fila en el frontend. */
  @Query(
      "SELECT c FROM McCuenta c WHERE c.empresaId = :empresaId AND c.cuentaPadreId.id = :padreId "
          + "ORDER BY c.id ASC")
  List<McCuenta> findHijos(
   ...
   */
  @Query(
      "SELECT DISTINCT c.cuentaPadreId.id FROM McCuenta c "
