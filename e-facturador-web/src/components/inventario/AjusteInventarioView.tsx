...
import { InMovimientoTipo } from "../../apis/InMovimientoTipoController";
import AjusteBuscarModal from "./AjusteBuscarModal";
...
const AjusteInventarioView: React.FC = () => {
    ...
    // cr=true → solo aumentar; cr=false → solo reducir; null → sin motivo seleccionado
    const [motivoCr, setMotivoCr] = useState<boolean | null>(null);
    ...
    /** Devuelve true si la línea viola la regla cr del motivo seleccionado. */
    const lineaViolaCr = (linea: LineaAjuste): boolean => {
        if (motivoCr === null) return false;
        const nueva = parseInt(linea.cantidadNueva, 10);
        if (isNaN(nueva)) return false;
        if (motivoCr && nueva < linea.cantidadActual) return true;   // cr=true: no puede reducir
        if (!motivoCr && nueva > linea.cantidadActual) return true;  // cr=false: no puede aumentar
        return false;
    };

    const handleGuardar = async () => {
        if (!almacenId) { showSnack("Seleccione un almacén", "error"); return; }
        if (!movimientoTipoId) { showSnack("Seleccione el tipo de movimiento", "error"); return; }
        if (lineas.length === 0) { showSnack("Agregue al menos un producto", "error"); return; }

        // Validar dirección según cr
        const violaciones = lineas.filter(lineaViolaCr);
        if (violaciones.length > 0) {
            const dir = motivoCr ? "aumentar" : "reducir";
            showSnack(`El motivo seleccionado solo permite ${dir} el stock. Revise las líneas marcadas en rojo.`, "error");
            return;
        }
    ...
    return (
        <>
                                ...
                                <Table size="small">
                                    ...
                                    <TableBody>
                                        {lineas.map((linea, idx) => {
                                            ...
                                            const viola = lineaViolaCr(linea);
                                            const bg = viola ? "#ffebee" : colorLinea(linea);
                                            const border = viola ? "#c62828" : borderLinea(linea);
                                            // min/max según cr del motivo
                                            const inputMin = motivoCr === true ? linea.cantidadActual : 0;
                                            const inputMax = motivoCr === false ? linea.cantidadActual : undefined;
                                            const tooltipError = viola
                                                ? (motivoCr ? "El motivo seleccionado solo permite aumentar el stock" : "El motivo seleccionado solo permite reducir el stock")
                                                : "Eliminar línea";
                                            return (
                                                <TableRow key={idx} hover>
                                                    ...
                                                    <TableCell align="right" sx={{ py: 0.5 }}>
                                                        <Tooltip
                                                            title={viola ? (motivoCr ? "Solo puede aumentar el stock con este motivo" : "Solo puede reducir el stock con este motivo") : ""}
                                                            open={viola}
                                                            placement="top"
                                                            arrow
                                                        >
                                                            <OutlinedInput
                                                                ...
                                                                size="small"
                                                                inputProps={{ min: inputMin, max: inputMax, step: "1" }}
                                                                error={viola}
                                                                sx={{
                                                                    width: 120,
                                                                    bgcolor: bg,
            ...
            <AjusteBuscarModal open={buscarOpen} onClose={() => setBuscarOpen(false)} />
